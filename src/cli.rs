...
pub fn prompt_song(songs: &[Song]) -> usize {
    loop {
        print!("Please enter the song ID or the Song Name and Artist like so: Name,Artist: ");
        ...
        if let Some(i) = songs.iter().position(|s| {
            s.track_id.eq_ignore_ascii_case(q)
                || q.eq_ignore_ascii_case(&format!("{},{}", s.track_name, s.artist_name)) //changed this
        }) {
